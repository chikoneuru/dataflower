...
class DataManager:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.cache_dir = Path(self.config.get('cache_dir', '/.tmp/dataflower_cache'))
    ...
    def get_data(self, request: DataAccessRequest) -> Tuple[Optional[Any], DataLocation]:
        ...
        # 2. For cross-node access, check remote storage
        if request.requesting_node != self.node_id or request.preferred_location == DataLocation.REMOTE_ONLY:
            ...
            if self.remote_storage:
                ...
                if remote_data is not None:
                    ...
                    # Optionally cache locally for future access
                    if request.requesting_node == self.node_id:
                        self._cache_from_remote(data_id, remote_data)
    ...
    def _cache_from_remote(self, data_id: str, data: Any) -> bool:
        """Cache data locally after fetching from remote storage"""
        if data_id in self.remote_data_registry:
            metadata = self.remote_data_registry[data_id]
            metadata.last_accessed = time.time()
            return self._store_local(data_id, data, metadata)
        return False
