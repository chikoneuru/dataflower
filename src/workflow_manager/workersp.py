...
class WorkerSPManager:
    ...
    def flow_data(self, request_id, workflow_name, dest_template_name, dest_block_name, dest_data_name, data_infos,
                  offset=None):
        print(f"🌊 FLOWING TO: {dest_template_name}.{dest_block_name} ({dest_data_name})")
        flow_start = time.time()
        
        output_type = data_infos['output_type']
        workflow_state = self.workflows_state[request_id]
        
        # Process the flow data (existing logic)
        ...
        # ✅ LOG DATA FLOW TO WORKFLOW_LATENCY DB
        repo.save_latency({
            'request_id': request_id,
            'template_name': dest_template_name,
            'block_name': dest_block_name + '_data_flow',
            'phase': 'use_container',
            'time': flow_time,
            'st': flow_start,
            'ed': flow_end,
            'cpu': 0.1,              # ← ADD THIS FIELD
            'memory': 50,            # ← ADD THIS FIELD
            'data_name': dest_data_name,
            'data_type': data_infos.get('datatype', 'unknown'),
            'data_size': data_infos.get('datasize', 0),
    ...
    def dispatch_incoming_data(self):
        ...
        if data.template_name == 'global_inputs':
            print(f"   📤 PROCESSING GLOBAL INPUT: {data.template_name}.{data.block_name}")
            st = time.time()
            ...
            print(f"   📤 PROCESSING TEMPLATE OUTPUT: {data.template_name}.{data.block_name}")
            already_transfered_ips = set()
            dest = None
            block_info = workflow_info.templates_infos[data.template_name]['blocks'][data.block_name]
            if 'output_datas' not in block_info:
                print(f"   ❌ NO OUTPUT_DATAS for {data.template_name}.{data.block_name}")
                return
                
            if data.data_name not in block_info['output_datas']:
                print(f"   ❌ OUTPUT {data.data_name} not found in block")
                return
            if block_info['type'] == 'SWITCH':
                dest = block_info['conditions'][data_infos['switch_branch']][data_name]
                print(f"   ➡️ DESTINATIONS for {data.data_name}: {dest}")
            ...
            for dest_template_name, dest_template_infos in dest.items():
                print(f"   🎯 PROCESSING DEST: {dest_template_name}")
                self.post_user_data(data.request_id, data.data_name, data_infos['val'])
                if dest_template_name == '$USER':
                    print(f"      📤 SENDING FINAL RESULT TO GATEWAY")
    ...
    def trigger_normal(self, request_id, workflow_name, template_name, block_name, input_datas=None):
        ...
        # Add logging BEFORE function execution
        execution_start = time.time()
        
        # ✅ LOG FUNCTION START TO WORKFLOW_LATENCY DB
        repo.save_latency({
            'request_id': request_id,
            'template_name': template_name,
            'block_name': block_name + '_start',
            'phase': 'use_container',
            'time': 0,
            'st': execution_start,
            'ed': execution_start,
        ...
        try:
            ...
            # Execute the function through function manager
            delay_time = self.function_manager.allocate_block(request_id, workflow_name, template_name,
            ...
            # ✅ LOG SUCCESSFUL FUNCTION COMPLETION TO WORKFLOW_LATENCY DB
            repo.save_latency({
                'request_id': request_id,
                'template_name': template_name,
                ...
                'memory': 100,  # Estimated memory usage
                'delay_time': delay_time if 'delay_time' in locals() else 0,
                'status': 'success'
            })
            ...
            repo.save_log({
                ...
                'st': execution_start,
                'ed': execution_end,
                'cpu': 0.5,  # hardcoded for now
                'memory': 100,  # hardcoded for now
                'delay_time': delay_time if 'delay_time' in locals() else 0,
                'status': 'success',
                'log_type': 'function_execution'
            })
            
            # ✅ UPDATE RESULTS DB WITH FUNCTION EXECUTION
            self.store_function_execution_result(request_id, workflow_name, template_name, 
                                               block_name, execution_time, delay_time, 'success')
            
            print(f"✅ Logged execution for {request_id} - {template_name}.{block_name}: {execution_time:.3f}s")
            
        except Exception as e:
            execution_end = time.time()
            execution_time = execution_end - execution_start
            
            # ✅ LOG FAILED FUNCTION EXECUTION TO WORKFLOW_LATENCY DB
            repo.save_latency({
                'request_id': request_id,
                'template_name': template_name,
                'block_name': block_name + '_error',
                'phase': 'use_container',
                'time': execution_time,
                'st': execution_start,
                'ed': execution_end,
                'cpu': 0.0,              # hardcoded for now
                'memory': 0,             # hardcoded for now
                'error': str(e),
                'status': 'failed'
            })
            
            # ✅ SAVE ERROR TO LOGS DB
            repo.save_log({
                'request_id': request_id,
                'template_name': template_name,
                'block_name': block_name + '_error',
            ...
            # ✅ UPDATE RESULTS DB WITH ERROR
            self.store_function_execution_result(request_id, workflow_name, template_name, 
                                               block_name, execution_time, 0, 'failed', str(e))
            
            print(f"❌ Logged error for {request_id} - {template_name}.{block_name}: {str(e)}")
            raise

    def store_function_execution_result(self, request_id, workflow_name, template_name, 
                                      block_name, execution_time, delay_time, status, error=None):
        """Store function execution results in results database"""
        try:
            ...
            # Initialize executions array if not exists
            if 'executions' not in result_doc:
                result_doc['executions'] = []
            
            # Add this function execution
            execution_record = {
                'template_name': template_name,
                'block_name': block_name,
                ...
                'status': status,
                'worker_ip': self.host_addr
            }
            
            if error:
                execution_record['error'] = error
            
            result_doc['executions'].append(execution_record)
            
            # Update document metadata
            result_doc.update({
                'workflow_name': workflow_name,
                'last_updated': current_time,
                'total_functions_executed': len(result_doc['executions']),
                'total_execution_time': sum(exec['execution_time'] for exec in result_doc['executions']),
                'status': 'running' if status == 'success' else 'failed'
            })
            
            # Save to database
            repo.couchdb['results'][request_id] = result_doc
            print(f"✅ Stored execution result for {request_id} - {template_name}.{block_name}")
            
        except Exception as e:
            print(f"❌ Error storing execution result for {request_id}: {e}")

    def send_data_remote(self, remote_addr, request_id, workflow_name, template_name, block_name, datas, from_virtual):
        remote_url = f'http://{remote_addr}:8000/transfer_data'
