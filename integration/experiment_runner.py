...
class ExperimentRunner:
    ...
    def _clear_minio_bucket(self) -> None:
        ...
        try:
            ...
            # MinIO configuration
            minio_host = 'localhost'  # Use localhost for local testing
            minio_port = 9000
...
def create_scenario_configs(
    ...
    scenarios = [
        ...
        },
        {
            ...
            "input_size_mb": 5,  # Fixed baseline
            "concurrency": [1, 10, 20], #, 50],  # VARIES: 1, 20, 100
            "request_rate_per_sec": 1,  # Fixed baseline
            "bandwidth_cap_mbps": 1000,  # No cap
            "injected_latency_ms": 1,
        ...
        },
        {
            ...
            "request_rate_per_sec": [10, 20, 50], # 100, 200],  # VARIES: 1, 50, 200 req/s
            "bandwidth_cap_mbps": 1000,  # No cap
            "injected_latency_ms": 1,
            "duration_sec": 10,  # Extended for higher rates
            "replicates": 1,
            "refinement_time_cap_sec": 0.02,
...
def ensure_small_msgs_input_data(input_sizes_mb: List[float], base_image_path: str = 'data/test.png') -> None:
    """
    Check whether input data files exist for Small-Msgs scenario and create them if needed.
    
    Args:
        input_sizes_mb: List of input sizes in MB (e.g., [0.01, 0.05, 0.1, 0.5, 1])
        base_image_path: Path to base image for downsampling (default: 'data/test.png')
    
    Raises:
        FileNotFoundError: If base image doesn't exist and files need to be created
    """
    import os
    
    print("🔍 Checking Small-Msgs input data files...")
    
    # Track which files need to be created
    missing_files = []
    existing_files = []
    
    for size_mb in input_sizes_mb:
        # Determine the expected file path
        input_file = _get_input_file_for_size(size_mb)
        
        if os.path.exists(input_file):
            actual_size_mb = os.path.getsize(input_file) / (1024 * 1024)
            existing_files.append((input_file, actual_size_mb))
            print(f"   ✅ Found: {input_file} ({actual_size_mb:.3f} MB)")
        else:
            missing_files.append((input_file, size_mb))
            print(f"   ❌ Missing: {input_file} ({size_mb} MB)")
    
    # If any files are missing, create them
    if missing_files:
        print(f"\n📦 Creating {len(missing_files)} missing input files...")
        
        # Check if base image exists for downsampling
        if not os.path.exists(base_image_path):
            raise FileNotFoundError(
                f"Base image not found: {base_image_path}\n"
                f"Please ensure the base image exists before creating downsampled images."
            )
        
        # Ensure data directory exists
        os.makedirs('data', exist_ok=True)
        
        for input_file, size_mb in missing_files:
            try:
                target_size_bytes = int(size_mb * 1024 * 1024)
                print(f"\n   Creating {input_file} (target: {size_mb} MB)...")
                
                # Use _create_downsampled_test_image for small sizes
                if size_mb < 5:
                    _create_downsampled_test_image(base_image_path, input_file, target_size_bytes)
                else:
                    # Use create_test_image which handles both small and large sizes
                    create_test_image(target_size_bytes, input_file)
                    
            except Exception as e:
                print(f"   ⚠️  Error creating {input_file}: {e}")
                # Try alternative method
                try:
                    create_test_image(target_size_bytes, input_file)
                except Exception as e2:
                    print(f"   ❌ Failed to create {input_file}: {e2}")
        
        print(f"\n✅ Finished creating input files for Small-Msgs scenario")
    else:
        print(f"\n✅ All {len(existing_files)} input files exist for Small-Msgs scenario")
...
def main():
    ...
    for i, arg in enumerate(sys.argv[1:], 1):
        ...
        elif arg == "--split-legacy":
            split_legacy = True
        elif arg == "--reorganize":
            split_legacy = True  # Use same flag for comprehensive reorganization
    ...
    # Handle legacy file splitting first
    if split_legacy:
        print("🔄 Comprehensive File Reorganization Mode")
        print("=" * 40)
        runner = ExperimentRunner()
        runner.reorganize_all_files()
        return 0
