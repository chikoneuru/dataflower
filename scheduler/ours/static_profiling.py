...
class CalibrationRunner:
    ...
    def _calibrate_functions(self, functions: List[str], input_sizes_mb: List[float], 
        ...
        for function_name in functions:
            ...
            # Use per-function input sizes
            function_input_sizes = self._get_function_input_sizes(function_name, input_sizes_mb)
            ...
            for node_for_test in nodes_with_func:
                for i, input_size_mb in enumerate(function_input_sizes, 1):
                    print(f"    🔍 Testing {input_size_mb}MB input ({i}/{len(function_input_sizes)}) on node {node_for_test}...")
    ...
    def _get_function_input_sizes(self, function_name: str, default_sizes: Optional[List[float]]) -> List[float]:
        """Get appropriate input sizes for a function based on its type"""
        text_functions = ['recognizer__translate', 'recognizer__censor']
        
        if function_name in text_functions:
            return [0.001, 0.005, 0.01, 0.05, 0.1]  # 1KB to 100KB
        else:
            # Filter default sizes to be >= 0.1 flowingB for image functions
            if default_sizes is None:
                # Default sizes for image functions if not provided
                return [0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10, 20, 50]
            return [size for size in default_sizes if size >= 0.1]
    ...
    def _send_raw_request_to_container(self, container, function_name: str, test_data: bytes, input_size_mb: float):
        ...
        try:
            ...
            if function_name in ["recognizer__translate", "recognizer__censor"]:
                ...
                prepared_input = {
                    "text": text_content,
                    "request_id": f'profile_{function_name}_{int(time.time() * 1000)}'
                }
            else:
                ...
                try:
                    ...
                    # Add request_id after getting dict from prepare_input_for_node
                    if isinstance(prepared_input, dict):
                        prepared_input['request_id'] = f'profile_{function_name}_{int(time.time() * 1000)}'
                except KeyError as e:
                    # If the shared logic fails, create a dict with img_b64
                    import base64
                    img_b64 = base64.b64encode(test_data).decode('utf-8')
                    prepared_input = {
                        "img_b64": img_b64,
                        "request_id": f'profile_{function_name}_{int(time.time() * 1000)}'
                    }
    ...
    def _measure_dag_execution(self, target_function: str, input_size_mb: float, samples: int) -> List[float]:
        ...
        for i in range(samples):
            try:
                ...
                request_id = f"profile_{target_function}_{i}_{int(time.time())}"
...
# Import math for scaling law fitting
import math
