...
class FunctionOrchestrator:
                    ...
                    if status_val != 'error':
                        try:
                            self._persist_function_output(
                                workflow_request_id=self._current_workflow_request_id if hasattr(self, '_current_workflow_request_id') else request_id,
                                function_name=function_name,
                                node_id=node_id,
    ...
    def _send_request_to_container(self, container: FunctionContainer, 
        ...
        # If body is None, this means the function should retrieve input from storage
        if body is None:
            # Determine the input key for this function based on dependencies
            input_key = self._determine_input_key_for_function(function_name, timeline)
            
            # Create storage context for the function - functions expect 'input_ref' key
            if input_key:
                # Use remote storage (MinIO) for data retrieval
                # Use centralized MinIO configuration for storage context
                minio_storage_config = get_minio_storage_config()
                storage_context = {
                    'input_ref': {
                        'src': 'remote',
                        'input_key': input_key,
                        'storage_config': minio_storage_config
                    }
                }
                self.logger.info(f"DEBUG: Sending storage context to {function_name}: {storage_context}")
            else:
                # No input available - this means the function should receive direct input
                # But since body is None, this indicates a logic error in the caller
                # The function should not be called with body=None if it has no dependencies
                self.logger.error(f"Function {function_name} called with body=None but has no dependencies - this should not happen")
                storage_context = {
                    'input_ref': {
                        'src': 'none',
                        'input_key': None
            ...
            if input_key:
                self.logger.info(f"\033[96mFunction {function_name} will retrieve input from storage: {input_key}\033[0m")
            else:
                # Check if this function has dependencies that aren't ready yet
                workflow_request_id = getattr(self, '_current_workflow_request_id', None)
                if workflow_request_id:
                    # Try to determine if this function has dependencies
                    active_workflow = None
                    for workflow_name, workflow_info in self.active_workflows.items():
                        if workflow_request_id.startswith(f"{workflow_name}_"):
                    ...
                    if active_workflow:
                        ...
                        if function_node:
                            ...
                            if has_dependencies:
                                self.logger.info(f"\033[93mFunction {function_name} has dependencies but they are not ready yet\033[0m")
                            else:
                                self.logger.info(f"\033[93mFunction {function_name} has no input dependency (first function)\033[0m")
                        else:
                            self.logger.info(f"\033[93mFunction {function_name} has no input dependency (first function)\033[0m")
                    else:
                        self.logger.info(f"\033[93mFunction {function_name} has no input dependency (first function)\033[0m")
            ...
            return OrchestratorUtils.send_request_to_container(container, function_name, storage_context, timeline, timeout=300)
        else:
            # Pass data directly for first function or when storage is not available
            ...
            if hasattr(self, 'workflow_results') and workflow_request_id in self.workflow_results:
                ...
                for completed_function in reversed(completed_functions):
                    if completed_function in dependency_functions:
                        input_key = f"{workflow_request_id}_{completed_function}_output"
                        return input_key
