...
class FunctionOrchestrator:
    ...
    def submit_workflow(self, workflow_name: str, request_id: str, body: Any) -> Dict:
        ...
        # ✅ FIXED (each request_id gets its own isolated context)
    ...
    def schedule_workflow(self, dag: Dict, context: Dict, request_id: str) -> Dict:
        ...
        with self._time_operation(timestamps_dict, 'workflow_orchestration'):
            ...
            for task_id in topological_order:
                ...
                # Create function request ID to match DAG executor pattern
                function_request_id = f"{request_id}_{function_name}"
                self._workflow_for_request[request_id] = workflow_name
                self._workflow_for_request[function_request_id] = workflow_name  # Also map function request ID
                
                # Determine storage context if function has dependencies
                storage_context = None
                if dependencies:
                    # Use the helper method to determine storage context
                    storage_context = self._determine_storage_context_for_function(function_name, function_request_id)
                    if storage_context:
                ...
                try:
                    result = self.invoke_function(function_name, function_request_id, input_data, storage_context=storage_context)
    ...
    def _determine_storage_context_for_function(self, function_name: str, request_id: str) -> Optional[Dict]:
        ...
        try:
            # Extract workflow request ID from the request_id parameter
            # IMPORTANT: Always extract from request_id to avoid race conditions with concurrent workflows
            # Pattern: request_id.rsplit('_', 1)[0] to remove the '_functionname' suffix
            if '_recognizer__' in request_id:
                # For recognizer workflow functions, extract everything before the function name
                # e.g., "vary-firingrate_FaasPRS_request_rate_per_sec10_workflow_0_recognizer__adult" 
                # should become "vary-firingrate_FaasPRS_request_rate_per_sec10_workflow_0"
                workflow_request_id = request_id.rsplit('_recognizer__', 1)[0]
                self.logger.debug(f"Extracted workflow_request_id from request_id: {workflow_request_id}")
            elif '_' in request_id:
                # Generic fallback: remove last component
                workflow_request_id = request_id.rsplit('_', 1)[0]
                self.logger.debug(f"Extracted workflow_request_id from request_id (generic): {workflow_request_id}")
            else:
                self.logger.warning(f"No active workflow found for function {function_name}, request_id: {request_id}")
                return None
            ...
            self.logger.debug(f"Looking for workflow with request_id: {workflow_request_id}")
            
            # Direct lookup by request_id key (new canonical mapping)
            active_workflow = self.active_workflows.get(workflow_request_id)
            if active_workflow:
                workflow_name = active_workflow.get('workflow_name') or 'unknown'
            else:
                # Fallback scan (legacy compatibility): search values for matching request_id
                for wf_name, wf_info in self.active_workflows.items():
                    if wf_info.get('request_id') == workflow_request_id:
                        active_workflow = wf_info
                        workflow_name = wf_info.get('workflow_name') or wf_name
                        break
            ...
            self.logger.debug(f"Found active workflow: {workflow_name} for request: {workflow_request_id}")
            ...
            if dependencies:
                ...
                self.logger.debug(f"Generated input_key for {function_name}: {input_key}")
                ...
                if input_key:
                    ...
                    storage_context = {
                        ...
                        'request_id': workflow_request_id  # Include request_id for output persistence
                    }
                    
                    self.logger.info(f"Function {function_name} will retrieve input from storage: {input_key}")
                    return storage_context
                else:
                    self.logger.warning(f"No input_key generated for {function_name} with dependencies: {dependencies}")
    ...
    def _determine_input_key_for_function(self, function_name: str, workflow_request_id: str, dependencies: List[str]) -> Optional[str]:
        ...
        try:
            self.logger.debug(f"Determining input key for {function_name} with workflow_request_id: {workflow_request_id}, dependencies: {dependencies}")
            
            if dependencies:
                # Use the first dependency as default
                dependency = dependencies[0]
                input_key = f"{workflow_request_id}_{dependency}"
                self.logger.debug(f"Default input_key for {function_name}: {input_key}")
                ...
                input_key = f"{workflow_request_id}_{dependency}"
                self.logger.debug(f"Final input_key for {function_name}: {input_key}")
                return input_key
    ...
    def invoke_function(self, function_name: str, request_id: str, body: bytes, node: Optional[str] = None, storage_context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        with self._time_overhead_operation(func_tl['timestamps'], 'timeline_setup'):
            ...
            try:
                # Extract workflow request ID from function request ID
                # IMPORTANT: Always extract from request_id to avoid race conditions with concurrent workflows
                if '_recognizer__' in request_id:
                    workflow_request_id = request_id.rsplit('_recognizer__', 1)[0]
                elif '_' in request_id:
                ...
                self.logger.debug(f"Added function timeline for {function_name} to workflow timeline: {workflow_request_id}")
            except Exception as e:
        ...
        with self._time_overhead_operation(func_tl['timestamps'], 'request_preparation'):
            ...
            if not container:
                ...
                # Extract workflow request ID from function request ID
                if '_recognizer__' in request_id:
                    root_id = request_id.rsplit('_recognizer__', 1)[0]
                elif '_' in request_id:
                    root_id = request_id.rsplit('_', 1)[0]
                else:
                    root_id = request_id
        ...
        start_time = time.time()
        try:
            # Determine storage context for function invocation if not provided
            ...
            with self._time_overhead_operation(func_tl['timestamps'], 'timeline_merging'):
                ...
                try:
                    # Extract workflow_request_id from function request ID
                    # IMPORTANT: Always extract from request_id to avoid race conditions with concurrent workflows
                    if '_recognizer__' in request_id:
                        workflow_request_id = request_id.rsplit('_recognizer__', 1)[0]
                    elif '_' in request_id:
                    ...
                    if workflow_request_id:
                        tl_root = self._get_or_create_timeline(workflow_request_id)
                        self._debug(f"Merging derived metrics for {function_name} into workflow timeline: {workflow_request_id}")
                        ...
                        self._debug(f"Skipping timeline merge - no workflow_request_id found for {request_id}")
                except Exception as e:
                    self._debug(f"Error updating workflow timeline: {e}")
            
            # Persist function output to storage
            if response.get('status') != 'error':
                # Extract workflow_request_id directly from the function request_id
                # The request_id is in format: workflow_N_recognizer__function
                # We need to extract workflow_N
                # IMPORTANT: Always extract from request_id to avoid race conditions with concurrent workflows
                workflow_request_id = None
                
                if '_' in request_id:
                    # For recognizer workflow functions, extract everything before the function name
                    # Expected format: <experiment_id>_<scheduler>_<params>_workflow_<N>_recognizer__<function>
                    if '_recognizer__' in request_id:
                        workflow_request_id = request_id.rsplit('_recognizer__', 1)[0]
                    else:
                        # Generic fallback: remove last component
                ...
                self.logger.debug(f"Extracted workflow_request_id: {workflow_request_id} from request_id: {request_id}")
                
                # Special logging for recognizer__upload to track persistence
                if function_name == 'recognizer__upload':
                    self.logger.info(f"DEBUG: Persisting recognizer__upload output for workflow_request_id: {workflow_request_id}")
                    self.logger.info(f"DEBUG: recognizer__upload response status: {response.get('status')}")
                    self.logger.info(f"DEBUG: recognizer__upload response keys: {list(response.keys()) if isinstance(response, dict) else 'Not a dict'}")
