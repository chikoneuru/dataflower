...
def main():
    ...
    if args.input_sizes:
        ...
        # Default input sizes - smaller for text functions to avoid timeouts
        # Check if we have text functions in the function list
        text_functions = ['recognizer__translate', 'recognizer__censor']
        has_text_functions = any(func in functions for func in text_functions)
        
        if has_text_functions:
            # Use smaller input sizes for text functions
            input_sizes_mb = [0.001, 0.005, 0.01, 0.05, 0.1] # 1KB, 5KB, 10KB, 50KB, 100KB
        else:
            # Use larger input sizes for image functions
            input_sizes_mb = [1.0, 2.0, 5.0, 10, 20, 50, 100] # 1MB, 2MB, 5MB, 10MB, 20MB, 50MB, 100MB
