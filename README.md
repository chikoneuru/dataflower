...
# This creates .tmp/python_pkgs_whls/ with all needed wheels
...
### 4. Calibrate Static Profiles
**Generate performance profiles** (required for accurate scheduling):
```bash
# Calibrate all functions with default settings
python -m scripts.calibrate_profiles

# Calibrate with custom input sizes
python -m scripts.calibrate_profiles --input-sizes 0.1,0.5,1.0,2.0,5.0

# Calibrate specific functions only
python -m scripts.calibrate_profiles --functions recognizer__adult,recognizer__violence

# Save with custom profile name
python -m scripts.calibrate_profiles --profile-name my_experiment --samples 10

# Expected: Profiles saved to results/static_profiles/default.json
# Shows batch-fitted coefficients (α, η) with R² quality metrics
```
...
curl -s -X POST http://localhost:8104/invoke -H 'Content-Type: application/json' --data-binary @<( { printf '{"img_b64":"'; base64 -w0 data/test.png; printf '"}'; } )
```
...
**Manage different schedulers** (Ours, FaasPRS, Ditto):
```bash
# Start shared infrastructure (Redis, MinIO, workers, function containers)
./scripts/manage_schedulers.sh start-warm-pools

# Run experiments with all schedulers
./scripts/manage_schedulers.sh run

# Run experiments with specific schedulers
./scripts/manage_schedulers.sh run ours,ditto

# Check container status
./scripts/manage_schedulers.sh status

# Stop all scheduler containers
./scripts/manage_schedulers.sh stop
```

### 7. Experiment Runner Commands

**Run scheduler comparison experiments**:
```bash
# 1. Quick smoke test first (verifies everything works)
python -m integration.experiment_runner smoke --log-level=DEBUG

# 2. If smoke test passes, run full scenarios, (skip completed ones)
...
python -m integration.experiment_runner test

# 5. Control logging
python -m integration.experiment_runner smoke --log-level=DEBUG

# 6. Override existing results (rerun all experiments)
python -m integration.experiment_runner --override
```

**Available Experiment Modes**:
- **`smoke`**: Quick verification (1 experiment, 10 requests, ~10s)
- **`test`**: Basic testing (3 experiments, 30 requests, ~30s)  
- **`standard`** (default): Full scenario-based experiments (15 experiments, ~98K requests)
- **`scale`**: Stress testing with large inputs and high concurrency

**Results are saved to**:
- `results/experiment_results.json` (all results combined)
- `results/timelines/ours_timelines.json` (by scheduler)
- `results/timelines/faasprs_timelines.json` (by scheduler)
- `results/timelines/ditto_timelines.json` (by scheduler)

**Per-Scenario Files** (when running standard mode):
- `results/baseline-cleannet_results.json`
- `results/bandwidth-limited_results.json`
- `results/burst_timelines.json`
- `results/high-concurrency_timeseries.csv`
- `results/small-msgs_results.json`
- And corresponding timeline and timeseries files for each scenario

**Resume & Override Features**:
- **Skip completed experiments**: Automatically skips scenarios that already have results
- **Override flag**: `--override` or `-f` forces rerunning all experiments
- **Incremental execution**: Perfect for running large experiments in batches
...
- **mock.json**: Mock storage and components for testing
- **simple.json**: Actual components with simple algorithms (random, round-robin)
- **production.json**: Production-ready settings with actual algorithms (heuristic, optimization)
...
./scripts/pre-download-tensorflow.sh
./scripts/pre-download-python-pkgs.sh
...
# Expected: Fast build using cached wheels instead of 590MB download
# Cache location: .tmp/tensorflow_cache/, .tmp/python_pkgs_whls/
```
