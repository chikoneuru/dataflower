...
def retrieve_input(input_ref: Dict, function_name: str = None) -> Union[bytes, Dict]:
	...
	if src == "local":
		...
		if not path:
			raise ValueError("Missing 'path' for local input source")
		full_path = os.path.join("/mnt/node_data", path)
		...
		if not input_key:
			raise ValueError("Missing 'input_key' for remote (MinIO) input source")
		...
		try:
			...
			raise ValueError(f"Failed to retrieve data from remote storage (MinIO): {e}")
	elif src == "none":
